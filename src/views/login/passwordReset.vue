    ...
    <el-form ref="passwordForm" :model="passwordForm" :rules="passwordRules" class="login-form" label-position="left">
      <div class="codeWarp">
        <el-form-item prop="emailcode">
          ...
          <el-input placeholder="验证码" name="emailcode" v-model.number="passwordForm.emailcode"></el-input>
        </el-form-item>
...
<script>
  import { postResetPassword, getEmailCode } from "@/api/login";
  export default {
    name: "PasswordReset",
    data() {
      const emailRules = {
      ...
      const passwordRules = {
        emailcode: [{ required: true, trigger: "blur", message: "验证码未填写" }],
        password: [{
          required: true,
          trigger: "blur",
          len: 6,
      ...
      return {
        emailForm: { email: "" },
        passwordForm: { emailcode: null, password: "" },
        emailRules,
        passwordRules,
        loading: false,
      ...
      getCode() {
        this.$refs["emailForm"].validate(valid => {
          if (valid) {
            ...
            getEmailCode({ ...this.emailForm, state: true }).then(({ data }) => {
              let type;
              if (data.status == 0) {
                type = "success";
              } else {
